...
COUNTRY_ID = str2double(parameters.country);                    % Country identificator.
if COUNTRY_ID == 214                                            % Select company identificator.
...
if UMA_ANALYSIS
    ...
    if networkcellinfo                                          % Filtering if Network Cell Info file included.
    ...
    [data_latitudes,data_longitudes,uma_grid_size,uma_sinr_data] = calculate_sinr_values_map(uma_transmitters,coordinates_bbox,prop_model);
...
if UMI_HOTSPOT
    ...
    if ~isempty(umi_cell_latitudes)
        ...
        if isempty(umi_transmitters)
            umi_transmitters = umi_transmitters_hotspot;
        else
...
if UMI_HOTSPOT || UMI_GRID || UMI_BLIND
    [umi_data_latitudes,umi_data_longitudes,best_umi_grid_size,umi_sinr_data] = calculate_sinr_values_map(umi_transmitters,coordinates_bbox,prop_model);
end
...
for t = 1:step:(total_time + 1)
    ...
    % Computing SINR for each receiver.
    rx_gain = 2.1;
    sinr_data = sinr(receivers, tx_analysis, ...
        'PropagationModel', propagation_model_selector(prop_model), ...
        'ReceiverGain', rx_gain);

    % Computing signal strength for each receiver.
    ...
    if ~sumo                                                    % Filtering if we have SUMO file.
        ...
        signal_receivers(time,:) = max(signal_strength_data);
    else
        sinr_receivers = sinr_data';                            % Current users reception data.
        signal_receivers = max(signal_strength_data);
    ...
    % If exists Network Cell Info file, store the metrics obtain with that mobile app for comparision.
    if networkcellinfo
    ...
    sinr_matrix = get_sinr_matrix_for_all_the_transmitters(receivers,tx_analysis,prop_model);
    count = 0;
...
if total_time~=0
    ...
    elseif sumo
        save('sumo_receivers_data.mat','sumo_sinr','sumo_signal','receivers_routes');
    end
