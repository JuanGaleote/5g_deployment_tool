...
for i = 1:length(COUNTRY_ID)
    if i == 1
        cells = readtable(['data_opencellid/' num2str(COUNTRY_ID(i)) '.csv']);
    else
        cells = [cells;readtable(['data_opencellid/' num2str(COUNTRY_ID(i)) '.csv'])];
    end
end
...
if CELL_ID ~= 0                                 % Filter if we have the CellID from the objective stations.
    ...
    for i=1:length(CELL_ID)
        ...
        if isempty(index)                       % If CellID unknown, request it to the API UnwiredLabs.     
            ...
            new_cell = {'LTE',COUNTRY_ID(1),COMPANY_ID,0,CELL_ID(i),0,response.lon,response.lat,0,0,0,0,0,0};
            cells = [cells;new_cell];
            writetable(cells,['data_opencellid/' num2str(COUNTRY_ID(1)) '.csv']);
